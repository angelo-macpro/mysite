...
for rowobj in rowlist :
    ...
    print(rowobj.serializable_value('companycode'),rowobj.serializable_value('companyname'))
print(comids)
...
def xltoxl(localpath):
    try:
        ...
        vname = 'V' + comid + '-' + str ( valid_date.date()) + '/' + str(dtoday.minute)
        # print(str(ws['B7'].value)) #原有版本号
        ws['B7'] = vname
        # print(str(ws['B7'].value)) #赋值后版本号
        print('你本次正在生成的填报文件版本号为：', vname)
        # 版本号写入EXCEL同时写入后台数据库,同时作为后续可否导入数据的检查
        models.Version.objects.get_or_create(accountdate=valid_date, isvalid=True, versionname=vname,
        ...
        for stname, obj in inspect.getmembers(models_view):
            if inspect.isclass(obj):
                ws = wb[stname]
                maxrowinxl = ws.max_row
                maxrowindb = obj.objects.filter(companycode=comid).count()
                if stname not in excludest and maxrowindb > maxrowinxl - 2:
                    diffs = obj.objects.filter(companycode=comid)[maxrowinxl-1:]
                    for rows in diffs.values_list():
                        ws.append(list(rows))
                # ws.sheet_state = 'hidden'
                ws.protection.enable()
        ...
        wb.save(filename)
        print('成功生成填报文件，文件保存位置为:'+os.getcwd()+' 文件名：'+filename)
